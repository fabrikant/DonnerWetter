	...
	function drawToCenter(dc, direction, arraySize){
		if (direction != -1){
			...
			for (var i = 0; i<windDirection.size(); i++){
				if (maxXY[0] < windDirection[i][0]){maxXY[0] = windDirection[i][0];}
				if (maxXY[1] < windDirection[i][1]){maxXY[1] = windDirection[i][1];}
				if (minXY[0] > windDirection[i][0]){minXY[0] = windDirection[i][0];}
				if (minXY[1] > windDirection[i][1]){minXY[1] = windDirection[i][1];}
			}
			...
			for (var i = 0; i<windDirection.size(); i++){
				windDirection[i] = [windDirection[i][0]+offsetX, windDirection[i][1]+offsetY];
			}
			dc.fillPolygon(windDirection);
		}
	}
