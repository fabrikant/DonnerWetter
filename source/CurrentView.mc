...
class CurrentView extends WatchUi.View {
  ...
  function onUpdate(dc) {
    ...
    var textW = Tools.max(
      dc.getTextWidthInPixels(wSpeed[:valueString], fontSmall),
      dc.getTextWidthInPixels(wSpeed[:unit], fontSmall)
    );
    ...
    dc.drawText(
      x - offset - 0.5,
      y + offset + dc.getFontHeight(fontSmall),
      fontSmall,
      field[0],
      Graphics.TEXT_JUSTIFY_RIGHT
    );
