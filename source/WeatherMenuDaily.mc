...
class WeatherMenuDaily extends WeatherMenu {
  function initialize() {
    ...
    var itemHeight =
      Graphics.getFontHeight(fontSmall) * 2 + Graphics.getFontHeight(fontMed);
...
class WeatherMenuItemDaily extends WeatherMenuItem {
  ...
  function draw(dc) {
    ...
    if (image != null) {
      ...
      var offset =
        (dc.getHeight() - imageH - 2 * Graphics.getFontHeight(fontSmall)) / 2;
      y = offset;
    ...
    xOffset = Tools.max(
      xOffset,
      x + dc.getTextWidthInPixels(tmpMax, fontSmall)
    );
    var tmpMin = getTemperature(data, TEMP_MIN);
    xOffset = Tools.max(
      xOffset,
      x + dc.getTextWidthInPixels(tmpMin, fontSmall)
    );
    //Temperature draw
    y = 0;
    ...
    wind.drawToCenter(dc, data[WIND_DEG], [
      x,
      y,
      dc.getWidth() - x,
      Graphics.getFontHeight(fontMed),
    ]);
    ...
    dc.drawText(
      x + (dc.getWidth() - x) / 2,
      Graphics.getFontHeight(fontMed) + Graphics.getFontHeight(fontSmall),
      fontSmall,
      wSpeed[:unit],
      Graphics.TEXT_JUSTIFY_CENTER
    );
